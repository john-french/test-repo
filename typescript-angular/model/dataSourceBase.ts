/**
 * Best Next Action API
 * APIs for Best Next Action capabilities.   Note that the error specification used by this API is based  on the IETF draft: https://tools.ietf.org/html/rfc7807 
 *
 * OpenAPI spec version: v1
 * 
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

import { ItemBase } from './itemBase';
import { NameValuePair } from './nameValuePair';


export interface DataSourceBase extends ItemBase {
    /**
     * Name of the Data Source Type to which this Data Source belongs
     */
    type?: string;
    /**
     * Collection of properties specific to this type of data source
     */
    properties?: Array<NameValuePair>;
}
